...
void Timer::createButtons() {
  ...
  lv_obj_align(btnMinutesUp, lv_scr_act(), LV_ALIGN_IN_TOP_LEFT, 20, 20);
  ...
  lv_obj_align(btnSecondsUp, lv_scr_act(), LV_ALIGN_IN_TOP_RIGHT, 20, 20);
  ...
  lv_obj_align(btnSecondsDown, lv_scr_act(), LV_ALIGN_IN_RIGHT_MID, 20, 40);
...
void Timer::OnButtonEvent(lv_obj_t* obj, lv_event_t event) {
  if (event == LV_EVENT_CLICKED) {
    if (obj == btnPlayPause) {
      ...
      bool timeControllerWasRunning = false;
      if (timerController.IsRunning()) {
        timeControllerWasRunning = true;
        uint32_t seconds = timerController.GetTimeRemaining() / 1000;
        minutesToSet = seconds / 60;
        secondsToSet = seconds % 60;
        timerController.StopTimer();
      ...
      if (!timerController.IsRunning() && timeControllerWasRunning) {
        timerController.StartTimer((secondsToSet + minutesToSet * 60) * 1000);
      }
    }
  }
